...
# load data
load('Data/bci01to24.5proc.rdata')
head(bciall)
# remove unwanted columns
bciall <- bciall[ ,-match(c('origorder', 'direction', 'notes', 'entrynotes', 'dendroHt', 
                            'paintDiam', 'paintHt', 'match.dendro'), 
                          colnames(bciall))]
# only keep the late censuses
bciall <- bciall[bciall$census %in% c(16,18,20,22,23,24), ]
...
for(i in 1:(n.census-1)){
  d1 <- m5[ ,grep(paste0('date.', i), colnames(m5))]
  d2 <- m5[ ,grep(paste0('date.', i+1), colnames(m5))]
